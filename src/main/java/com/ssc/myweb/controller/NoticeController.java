...
import com.ssc.myweb.util.PageVO;
...
public class NoticeController {
	...
	public String notice_list(Criteria cri, Model model) {
		...
		/*
		 * service, mapper영역에 getlist함수를 선언 등록번호를 역순으로 데이터를 조회해서 가지고 나가기 model에 담아서 화면에는
		 * 반복문으로 처리
		 */
		// 데이터
		...
		/*
		 * //Pagination 
		 * int total = tripService.getTotal(); PageVO pageVO = new
		 * PageVO(cri, total전체 게시글 수 );
		 */
		 ...
		 * 3. sql를 바꾼다 (동적쿼리)
		 * 4. total SQL문을 변경한다 (동적쿼리)
		 * 5. list페이지의 a링크 클릭시 searchType, searchName을 쿼리 스트링으로 넘긴다.
		 * 6. 검색 키워드 유지
		*/
	...
	public String notice_view(@RequestParam("tno") int tno, Model model, HttpServletResponse response,
...
//	}
	...
	public String deleteForm(@RequestParam("tno") int tno, RedirectAttributes ra) {
		...
		/*
		 * service , mapper 에는 noticeDelete메서드로 삭제를 진행 삭제 이후엔 list화면으로 이동
		 */
